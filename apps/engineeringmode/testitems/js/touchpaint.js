...
var TouchpaintTest = {
  ...
  clickHandler: function tpt_clickHandler(evt) {
    ...
    if (testcase == 'corner-test') {
      var checked = document.body.querySelectorAll('.corner-target[data-touched=true]').length;
      if (checked < 5) {
        return;
      }
      this.attachEdgeBtns();
    } else {
      var checked = document.body.querySelectorAll('.edge-target[data-touched=true]').length;
      if (checked < 28) {
        return;
      }
  ...
  attachCornerBtns: function tpt_attachCornerBtns() {
    ...
    for (var i = 0; i < positionList.length; i++) {
      var node = document.getElementsByClassName('corner-touch-area')[0].cloneNode();
  ...
  attachEdgeBtns: function tpt_attachEdgeBtns() {
    ...
    for (var i = 0; i < 6; i++) {
      var topNode = document.getElementsByClassName('edge-touch-area')[0].cloneNode();
      var bottomNode = document.getElementsByClassName('edge-touch-area')[0].cloneNode();
      topNode.style.left = bottomNode.style.left = 'calc((20% - 1rem) * ' + i + ')';
      topNode.style.top = '0';
    ...
    for (var i = 0; i < 8; i++) {
      var leftNode = document.getElementsByClassName('edge-touch-area')[0].cloneNode();
      var rightNode = document.getElementsByClassName('edge-touch-area')[0].cloneNode();
      leftNode.style.top = rightNode.style.top = 'calc((100% - 5rem) / 9 * ' + ( i + 1 ) + ')';
